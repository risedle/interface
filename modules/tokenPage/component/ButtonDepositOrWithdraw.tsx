 ...
 */
const ButtonDepositOrWithdraw: FunctionComponent<ButtonDepositOrWithdrawProps> = ({ chainID, address }) => {
    const metadata = Metadata[chainID][address];

    return (
        <Dialog.Root>
            <Dialog.Trigger className="w-full rounded-full border border-blue-light-11 bg-blue-light-10 py-[11px] text-sm font-semibold leading-4 tracking-[-0.02em] text-gray-light-1 dark:border-blue-dark-11 dark:bg-blue-dark-10 dark:text-blue-light-1">Deposit or Withdraw</Dialog.Trigger>
                ...
                <div className="mx-auto mb-4 flex-col rounded-3xl border border-gray-light-3 bg-gray-light-1 p-4 dark:border-gray-dark-3 dark:bg-gray-dark-1 sm:m-auto sm:max-w-[376px] sm:flex-auto">
                    <Dialog.Title className="mb-4 flex flex-row items-center justify-between">
                        <div className="flex flex-row items-center space-x-4">
                            <div>
                                <img src={metadata.vaultLogo} alt={metadata.vaultTitle} className="h-12 w-12" />
                            </div>
                            <div>
                    ...
                    <Tabs.Root defaultValue="deposit" className="outline-0">
                        <Tabs.List aria-label="depositOrWithdraw" className="mx-auto mb-6 mt-2 flex flex-row rounded-xl bg-gray-light-3 p-1 dark:bg-gray-dark-2">
                            <Tabs.Trigger value="deposit" className="basis-1/2 rounded-lg text-sm leading-4 text-gray-light-10 state-active:bg-gray-light-1 state-active:py-3 state-active:font-bold state-active:text-gray-light-12 dark:text-gray-dark-10 state-active:dark:bg-gray-dark-4 state-active:dark:text-gray-dark-12">
                                Deposit
                            </Tabs.Trigger>
                            <Tabs.Trigger value="withdraw" className="basis-1/2 rounded-lg text-sm leading-4 text-gray-light-10 state-active:bg-gray-light-1 state-active:py-3 state-active:font-bold state-active:text-gray-light-12 dark:text-gray-dark-10 state-active:dark:bg-gray-dark-4 state-active:dark:text-gray-dark-12">
                                Withdraw
                            </Tabs.Trigger>
                        </Tabs.List>

                        <Tabs.Content value="deposit" className="mx-auto flex flex-col space-y-6 outline-0 sm:max-w-[540px]">
                            <DepositDialogContent chainID={chainID} address={address} />
                        </Tabs.Content>
                        <Tabs.Content value="withdraw" className="mx-auto flex flex-col space-y-6 outline-0 sm:max-w-[540px]">
                            <WithdrawDialogContent chainID={chainID} address={address} />
                        </Tabs.Content>
                    </Tabs.Root>
                </div>
            </Dialog.Content>
