...
function ForthJumobtronBox({ children, className }: { className?: string; children?: React.ReactNode }) {
    return <div className={`relative mt-4 h-[236px] max-w-full overflow-hidden rounded-3xl bg-dark-background-elevated pt-8 pl-8 text-left sm:h-[280px] xl:h-[300px]     ${className || ""}`}>{children}</div>;
}
...
function Image({ src, alt }: { src: string; alt: string }) {
    return (
        <>
            <img src={src} alt={alt} className="absolute left-[32px] bottom-[-80px] h-full w-full  rounded-lg sm:bottom-0 sm:h-[210px]  " />
