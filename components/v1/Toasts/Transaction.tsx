 ...
 */
const ToastTransaction: FunctionComponent<ToastTransactionProps> = ({ hash, children }) => {
    // Get explorer link
    const { chain } = useWalletContext();

    // Get tx link
    let txLink, simplifiedHash;
    if (hash) {
        txLink = chain.blockExplorers && chain.blockExplorers.length ? `${chain.blockExplorers[0].url}/tx/${hash}` : "#";
        // Example hash 0x4bd32945f8d1ee82a24422074460b8e811b200eec1b0d6264e8fd171a0b50c89
        simplifiedHash = `${hash.slice(0, 6)}...${hash.slice(-4)}`;
    }
    return (
