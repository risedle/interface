...
const ButtonLoading: FunctionComponent<ButtonLoadingProps> = ({ full, children }) => {
    return (
        <button onClick={(e) => e.preventDefault()} className={`cursor-wait flex flex-row text-center justify-center space-x-2 bg-gray-light-2 dark:bg-gray-dark-2 border border-gray-light-4 dark:border-gray-dark-4 text-blue-dark-1 dark:text-blue-light-1 text-sm leading-4 font-semibold py-[11px] px-4 rounded-full leading-4 inline-block tracking-[-0.02em] ${full ? "w-full" : ""}  drop-shadow-[0_0_45px_rgba(54,158,255,0.1)]`}>
