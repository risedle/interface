...
export function useTransactionHistory(req: TransactionHistoryRequest) {
    const { data, error } = useSWR<Array<TransactionHistoryResult>, Error>({ account: req.account, contract: req.contract, provider: req.provider, namespace: SWRCacheNamespace.TransactionHistory }, TransactionHistoryFetcher);
    return {
        data: data,
