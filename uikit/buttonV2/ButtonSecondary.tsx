...
const ButtonSecondary: FunctionComponent<ButtonSecondaryProps> = ({ onClick, size = "md", type = "default", children, className, ...restProps }) => {
    ...
    return (
        <button className={`flex flex-row items-center justify-center gap-2 active:scale-90 ${className} ${extraClass} bg-light-neutral-subtle/10 py-3 text-dark-neutral-primary transition ease-out ${type == "square" ? "rounded-lg" : "rounded-full"} ${restProps.disabled ? "opacity-40" : "hover:bg-light-neutral-subtle/20"}`} onClick={onClick} {...restProps}>
            {children}
        </button>
    );
};
